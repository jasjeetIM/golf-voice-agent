...
export function createAgentSession({ agent, transport, callSid, logger }: SessionDeps) {
  ...
  session.on('partial_transcript', (payload: any) =>
    log('partial_transcript', { callSid, ...payload })
  );
  session.on('final_transcript', (payload: any) =>
    log('final_transcript', { callSid, ...payload })
  );
