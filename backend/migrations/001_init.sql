...
CREATE TABLE IF NOT EXISTS provider_sessions (
  provider_session_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS provider_sessions_call_id_idx ON provider_sessions(call_id);
CREATE INDEX IF NOT EXISTS provider_sessions_external_session_id_idx ON provider_sessions(external_session_id);
CREATE INDEX IF NOT EXISTS provider_sessions_component_provider_idx ON provider_sessions(component, provider_name);

-- 8) call_events (transport/webhook stream events)
...
CREATE INDEX IF NOT EXISTS call_events_event_name_idx ON call_events(event_name);
CREATE INDEX IF NOT EXISTS call_events_payload_gin ON call_events USING GIN (payload_json);

-- 9) session_events (provider normalized events)
CREATE TABLE IF NOT EXISTS session_events (
  event_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  call_id TEXT NOT NULL REFERENCES calls(call_id),
  provider_session_id UUID REFERENCES provider_sessions(provider_session_id),
  component TEXT NOT NULL,
  provider_name TEXT NOT NULL,
  agent_name TEXT,
  event_name TEXT NOT NULL,
...
-- 10) conversation_items (provider-agnostic item state)
CREATE TABLE IF NOT EXISTS conversation_items (
  conversation_item_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  call_id TEXT NOT NULL REFERENCES calls(call_id),
  provider_session_id UUID REFERENCES provider_sessions(provider_session_id),
...
CREATE UNIQUE INDEX IF NOT EXISTS conversation_items_unique_external_idx
  ON conversation_items(call_id, provider_session_id, external_item_id)
  WHERE external_item_id IS NOT NULL AND provider_session_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS conversation_items_call_id_idx ON conversation_items(call_id);
CREATE INDEX IF NOT EXISTS conversation_items_tool_call_id_idx ON conversation_items(tool_call_id);

-- 11) tool_calls (canonical tool usage log)
...
CREATE INDEX IF NOT EXISTS mcp_calls_tool_name_idx ON mcp_calls(tool_name);
