...
class ReservationStore:
    ...
    async def modify(
        ...
        *,
        ...
        if changes.get("players") is not None and changes.get("players") != current_players:
            ...
            if delta > 0:
                if slot_row["players_booked"] + delta > slot_row["capacity_players"] or slot_row[
                    "is_closed"
                ]:
        ...
        await conn.execute(
            """
            INSERT INTO reservation_changes (reservation_id, change_type, call_id, after_json)
            SELECT reservation_id, $1, $2, $3
            FROM reservations
    ...
    async def cancel(
        ...
        await conn.execute(
            ...
            json.dumps(
                {"status": "CANCELLED", "confirmation_code": confirmation_code}
            ),
        )
