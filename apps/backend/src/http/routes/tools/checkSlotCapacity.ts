...
import { CheckSlotCapacityRequestSchema, CheckSlotCapacityResponseSchema } from "@golf/shared-schemas";
...
export function registerCheckSlotCapacity(app: FastifyInstance, { inventory }: Deps) {
  app.post(
    ...
    async (req: FastifyRequest<{ Body: unknown }>, reply: FastifyReply) => {
      ...
      const { rows } = await pool.query(`SELECT * FROM tee_time_slots WHERE slot_id = $1`, [parsed.data.slot_id]);
      ...
      const available =
        !slotRow.is_closed && slotRow.players_booked + parsed.data.players <= slotRow.capacity_players;
